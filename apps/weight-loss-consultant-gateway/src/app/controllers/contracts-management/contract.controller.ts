...
export class ContractController {
  ...
  @Put('/v1/contracts/proceedToCancel/:id')
  @UseGuards(JwtAuthGuard)
  async proceedToCancel(@Param('id') id: number, @Res() res) {
    try {
      const result = await this.contractService.proceedToCancel(id);
      res.status(HttpStatus.OK).send(result);
    } catch ({error}) {
      this.logger.error(error);
      res.status(error.statusCode).send(error);
  ...
  @Put('/v1/contracts/undoCancelContract/:id')
  @UseGuards(JwtAuthGuard)
  async undoCancelContract(@Param('id') id: number, @Res() res) {
    try {
      const result = await this.contractService.undoCancelContract(id);
      res.status(HttpStatus.OK).send(result);
    } catch ({error}) {
      this.logger.error(error);
      res.status(error.statusCode).send(error);
