export const GET_ALL_ADMINS = "get-all-admins";
export const GET_ADMIN_BY_EMAIL = "get-by-email";
export const CREATE_ADMIN = "create-admin";
export const UPDATE_ADMIN = "update-admin";
export const DELETE_ADMIN = "delete-admin";
export const UPDATE_ADMIN_WITHOUT_PASSWORD_AND_STATUS = 'update-admin-without-password-and-status'

export const GET_ALL_CUSTOMER = "get-all-customers";
export const VIEW_DETAIL_CUSTOMER = "view-detail-customer";
export const VIEW_DETAIL_SPECIAL = "view-detail-special";
export const UPDATE_CUSTOMER = "update-customer";
export const DELETE_CUSTOMER = "delete-customer";
export const CREATE_CUSTOMER = "create-customer";
export const UPDATE_CUSTOMER_WITHOUT_PASSWORD_AND_STATUS = 'update-customer-without-password-and-status'

export const GET_ALL_TRAINERS = 'get-all-trainers';
export const GET_TRAINER_BY_EMAIL = 'get-trainer-by-email';
export const CREATE_TRAINER = 'create-trainer';
export const UPDATE_TRAINER = 'update-trainer';
export const DELETE_TRAINER = 'delete-trainer';
export const VIEW_DETAIL_SPECIAL_TRAINER = 'view-detail-special-trainer'
export const UPDATE_TRAINER_WITHOUT_PASSWORD_AND_STATUS = 'update-trainer-without-password-and-status'

export const SORTING_AND_FILTERING_USERS = 'sorting-and-filtering-users';
export const SEARCH_USERS = 'search-users';

export const ADMIN_VIEW_DETAIL = 'admin-view-detail';
export const CUSTOMER_VIEW_DETAIL = 'customer-view-detail';
export const TRAINER_VIEW_DETAIL = 'trainer-view-detail';

export const UPDATE_PASSWORD = 'update-password';
export const UPDATE_STATUS = 'update-status';
